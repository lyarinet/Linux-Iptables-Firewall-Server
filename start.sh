...
$IPT -A INPUT -i eno3 -m state --state ESTABLISHED,RELATED -j ACCEPT
$IPT -A OUTPUT -o eno3 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
...
$IPT -A INPUT -p tcp --dport 1234 -m state --state NEW -j ACCEPT
$IPT -A INPUT -p tcp --dport 8880 -m state --state NEW -j ACCEPT
$IPT -A INPUT -p tcp --dport 445 -m state --state NEW -j ACCEPT
$IPT -A INPUT -p tcp --dport 7999 -m state --state NEW -j ACCEPT
$IPT -A INPUT -p tcp --dport 31210 -m state --state NEW -j ACCEPT

# $IPT -A INPUT -p tcp --destination-port 139 -j ACCEPT
# $IPT -A INPUT -p tcp --destination-port 8880 -j ACCEPT
# $IPT -A INPUT -p tcp --destination-port 445 -j ACCEPT
# $IPT -A INPUT -p tcp --destination-port 7999 -j ACCEPT
# $IPT -A INPUT -p tcp --destination-port 31210 -j ACCEPT
##### END your rules ############
 
# Do not log smb/windows sharing packets - too much logging
# $IPT -A INPUT -p tcp -i eno3 --dport 137:139 -j REJECT
# $IPT -A INPUT -p udp -i eno3 --dport 137:139 -j REJECT
